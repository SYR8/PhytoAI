...
10. **Live activation/execution state — audited 2026-09-15 (`docs/cross-audit-2026-09-15.md` §3):** live `phytoai` = **168 nodes / 182 edges, identical to the repo export**, `active:false`; the instance's other 8 workflows are unrelated personal projects; **no MCP/M5/Flask/serial nodes exist live or in the export** (the MCP-server/M5Stack/serial architecture from the audit brief was never materialized in this repo). Test executions exist (`691`–`725`); device executions UNKNOWN.
...
17. **`GET /config` missing preheat keys** — Plan §2.2 says `preheat_margin_c` / `preheat_lead_minutes` are exposed via `GET /config`, but `Build Config Response` does not include them (audit M3). Fix on the next workflow edit or amend Plan; the production WROOM preheat logic needs the decision.
18. **Source-of-truth mismatch (pump / water-temp)** — `max_pump_seconds` (sheet `60`) is never read (`Safety Guardrails` hardcodes `60`) and `max_water_temp_c` (sheet `28`) is ignored (`Safety Guardrails` hardcodes `30`) — audit M1/M2, `workflows/phytoai.json:3921`. Decide: read the keys or re-seed + document the override (safety-relevant for the production WROOM spec).
19. **Legacy `current_cycle_week` in `GET /config`** — Plan §2.2 removed the key but `Build Config Response` still returns `current_cycle_week: 'mapping'` (`workflows/phytoai.json:4642`; audit M5). Remove on the next workflow edit.
20. **Docs drift after Change 10** — README's Branch C narrative (L57–L68, L85) and Plan §2.3's `scan_position` example still describe the removed human gate; refresh both (audit M4).
21. **Duplicate reference export** — untracked `docs/PhytoAI-final.xlsx` is content-identical to `docs/PhytoAI.v2.xlsx` (audit M6/§5); keep v2 authoritative, then delete or label the duplicate.
