...
2. **The live Branch C still gates the scan on a human.** `Wait Camera Positioned` (60 min) +
   `scan_session_active` set only after dashboard confirmation. An autonomous `/yolo-scan` therefore receives
   `{"status":"no_active_session"}` until the owner confirms (or until the documented gate removal is applied).
   The firmware retries every 10 min for up to 90 min, then records a weekly miss. *Full autonomous scans
   require the documented Change 10 workflow edit — no workflow change was made here.*
...
| 5 | `POST /yolo-scan` scheduled scan | serial `s`; 200 `{"status":"ok","judge_verdict":…}`; DiseaseScans row appended | PENDING E2E (blocked by contract note 2 until gate removed/confirmed) |
...
- Apply/coordinate the documented workflow changes for autonomous scans (contract notes 1 and 2).
